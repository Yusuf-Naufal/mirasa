...
<div x-data="{ openCardId: null, openDropdownId: null }" class="space-y-4 md:hidden">
            ...
            <div class="flex justify-between items-start gap-3">
                ...
                <div class="flex gap-3 cursor-pointer flex-1 min-w-0"
                    ...
                    <div class="flex-shrink-0">
                        {{-- AVATAR USER --}}
                        <div
                            class="h-14 w-14 rounded-full overflow-hidden border-2 border-gray-100 shadow-sm bg-gray-50 flex items-center justify-center">
                            @if ($i->foto)
                                <img src="{{ asset('storage/' . $i->foto) }}" alt="{{ $i->name }}"
                                    class="h-full w-full object-cover">
                            @else
                                <div
                                    class="bg-green-100 text-green-600 h-full w-full flex items-center justify-center font-bold text-xl uppercase">
                                    {{ substr($i->name, 0, 1) }}
                                </div>
                            @endif
                        </div>
                                ...
                                @can('roles.delete')
                                    <li>
                                        <form id="delete-form-{{ $i->id }}"
                                            action="{{ route('roles.destroy', $i->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
            ...
            <div x-show="openCardId === {{ $i->id }}" x-collapse x-cloak
                ...
                <div class="grid grid-cols-1 gap-2">
                    <div class="flex justify-between items-center gap-1">
                        <span class="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Jumlah Akses:</span>
                        <span class="text-xs text-gray-700 font-medium uppercase">10</span>
                    </div>
                    <div class="flex justify-between items-center gap-1">
    ...
    @empty
        <div class="text-center py-10 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
            <p class="text-sm text-gray-500 font-medium">Data role tidak ditemukan</p>
            @can('roles.create')
                <a href="{{ route('roles.create') }}"
                    class="mt-3 inline-block text-green-600 text-xs font-bold hover:underline uppercase">Tambah Role
                    Baru</a>
            @endcan
        </div>
    @endforelse
