<x-layout.user.app title="Daftar Produk">
            ...
            <div class="flex w-full flex-col gap-3 md:w-auto md:flex-row md:items-center md:justify-between">
                <div class="flex gap-2">
                    <a type="button" href="{{ route('produk.create') }}"
                        ...
                        <span class="hidden md:block md:ml-2">Tambah Produk</span>
                    </a>
                    ...
                    <button onclick="openModal('filterModal')"
                        ...
                        <span class="hidden md:block md:ml-2">Filter Produk</span>
                    </button>

                </div>

                <form action="{{ route('produk.index') }}" method="GET" class="relative w-full md:w-64">
    ...
    <div id="filterModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md rounded-2xl bg-white shadow-2xl">
            ...
            <form action="{{ route('produk.index') }}" method="GET" class="p-6">
                {{-- Simpan nilai search agar tidak hilang --}}
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <select name="status"
                            ...
                            <option value="aktif" {{ request('status') == 'aktif' ? 'selected' : '' }}>Aktif (Tampil)
                            </option>
                            <option value="tidak_aktif" {{ request('status') == 'tidak_aktif' ? 'selected' : '' }}>Draft
                                (Sembunyi)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Produk Unggulan</label>
                        <select name="is_unggulan"
                            class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm outline-none focus:border-yellow-500">
                            <option value="">Semua</option>
                            <option value="ya" {{ request('is_unggulan') == 'ya' ? 'selected' : '' }}>Ya (Unggulan)
                            </option>
                            <option value="tidak" {{ request('is_unggulan') == 'tidak' ? 'selected' : '' }}>Tidak
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <div class="mt-8 flex items-center gap-3">
                    ...
                    <button type="submit"
                        class="flex-1 rounded-xl bg-gray-800 py-3 text-sm font-bold text-white hover:bg-black transition-colors shadow-sm">
                        Terapkan Filter
                    </button>
                </div>
            </form>
