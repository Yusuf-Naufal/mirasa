<x-layout.user.app>
    <div class="py-2">
        {{-- Ganti route ke .update dan sertakan ID barang --}}
            ...
            <div class="p-6 md:p-8 space-y-8">
                <div class="space-y-6">
                    <div class="flex items-center gap-2 pb-2 border-b border-gray-100">
                        <div class="p-2 bg-amber-50 rounded-lg">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-amber-500" viewBox="0 0 24 24">
                                <path fill="currentColor" d="m14.06 9.02l.92.92L5.92 19H5v-.92l9.06-9.06M17.66 3c-.25 0-.51.1-.7.29l-1.83 1.83l3.75 3.75l1.83-1.83c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.2-.2-.45-.29-.71-.29m-3.6 3.19L3 17.25V21h3.75L17.81 9.94l-3.75-3.75Z"/>
                            </svg>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div class="md:col-span-2 space-y-3">
                            ...
                            <div class="flex items-center gap-6">
                                <div id="preview-container"
                                    ...
                                    @if($barang->foto)
                                        <img id="img-preview" src="{{ asset('storage/' . $barang->foto) }}" class="object-cover w-full h-full">
                                        <svg id="placeholder-icon" class="hidden w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-xs text-gray-400 mt-2 italic">*Kosongkan jika tidak ingin mengubah foto</p>
                                </div>
                            </div>
                        </div>

                        <div class="space-y-1">
                            <label for="nama_barang" class="block text-sm font-semibold text-gray-700">Nama Barang <span class="text-red-500">*</span></label>
                        ...
                        <div class="space-y-1">
                            <label for="satuan" class="block text-sm font-semibold text-gray-700">Satuan Barang <span class="text-red-500">*</span></label>
                        ...
                        <div class="md:col-span-2 space-y-1">
                            <label for="id_perusahaan" class="block text-sm font-semibold text-gray-700">Perusahaan <span class="text-red-500">*</span></label>
                            <select id="id_perusahaan" name="id_perusahaan" required
                                ...
                                @foreach ($perusahaan as $p)
                                    <option value="{{ $p->id }}" {{ $barang->id_perusahaan == $p->id ? 'selected' : '' }}>{{ $p->nama_perusahaan }}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="space-y-1">
                            <label for="id_jenis" class="block text-sm font-semibold text-gray-700">Jenis Barang <span class="text-red-500">*</span></label>
                            <select id="id_jenis" name="id_jenis" required onchange="updateKodeJenis(this)"
                                class="w-full rounded-xl border-gray-300 py-2.5 px-4 shadow-sm focus:outline-none focus:border-[#FFC829] transition-colors border bg-white cursor-pointer">
                                @foreach ($jenis as $j)
                                    <option value="{{ $j->id }}" data-kode="{{ $j->kode }}" {{ $barang->id_jenis == $j->id ? 'selected' : '' }}>
                                        {{ $j->nama_jenis }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="space-y-1">
                            <label for="kode" class="block text-sm font-semibold text-gray-700">Kode Barang <span class="text-red-500">*</span></label>
                            <div class="relative flex">
                                {{-- Kita ambil kode murni setelah tanda - --}}
                                @php
                                    $kodeMurni = strpos($barang->kode, '-') !== false ? explode('-', $barang->kode)[1] : $barang->kode;
                                    $prefixAwal = strpos($barang->kode, '-') !== false ? explode('-', $barang->kode)[0] : '?';
                                @endphp
                                <span id="prefix-kode"
    ...
    <div id="cropper-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black/75 p-4">
        <div class="bg-white rounded-2xl max-w-2xl w-full overflow-hidden">
            <div class="p-4 border-b flex justify-between items-center">
                ...
                <button type="button" onclick="closeCropper()" class="text-gray-400 hover:text-gray-600">&times;</button>
            </div>
            <div class="p-4">
            ...
            <div class="p-4 border-t flex justify-end gap-3">
                <button type="button" onclick="closeCropper()" class="px-4 py-2 text-gray-600 font-semibold">Batal</button>
                <button type="button" onclick="cropAndSave()" class="px-6 py-2 bg-green-500 text-white rounded-xl font-bold hover:bg-green-600">Simpan Potongan</button>
            </div>
        </div>
    </div>
