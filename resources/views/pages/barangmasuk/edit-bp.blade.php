<x-layout.beranda.app title="Edit Barang Masuk - Bahan Penolong">
    <div class="min-h-screen bg-gray-50/50 md:px-10 py-8">
        <div class="mx-auto flex flex-col pt-12" x-data="{
            // Data Input
            jumlah: {{ $item->jumlah_diterima ?? 0 }},
            rusak: {{ $item->jumlah_rusak ?? 0 }},
            return_qty: {{ $item->jumlah_return ?? 0 }},
            ada_rusak: {{ ($item->jumlah_rusak > 0 || $item->jumlah_return > 0) ? 'true' : 'false' }},
            harga: {{ $item->harga ?? 0 }},
            ...
            get stok() { 
                ...
                let rusakInternal = this.ada_rusak ? (parseFloat(this.rusak) || 0) : 0;
                let rusakReturn = this.ada_rusak ? (parseFloat(this.return_qty) || 0) : 0;
            ...
            supplierOpen: false,
            supplierSearch: '',
            selectedSupplierId: '{{ $item->id_supplier }}',
            selectedSupplierName: '{{ $item->supplier->nama_supplier ?? '' }}',
            ...
            <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">
                        Edit Barang Masuk: <span class="text-yellow-600">Bahan Penolong</span>
                    </h1>
                </div>
            </div>
                            ...
                            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-3xl p-6 border border-blue-100/50">
                                ...
                                <div class="mb-4 space-y-2 relative z-[50]">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div x-show="supplierOpen" @click.away="supplierOpen = false"
                                            class="absolute z-[100] w-full mt-2 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden" x-cloak>
                                            <div class="p-2 border-b border-gray-50 bg-gray-50/50">
                                                <input type="text" x-model="supplierSearch" placeholder="Cari supplier..."
                                                    class="w-full px-4 py-2 text-sm bg-white border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-blue-500">
                                            </div>
                                ...
                                <div class="space-y-4 relative z-[40]">
                                        ...
                                        <div class="relative">
                                            ...
                                            <button type="button" @click="barangOpen = !barangOpen; supplierOpen = false"
                                                ...
                                                <span :class="selectedBarangName ? 'text-gray-700 font-medium' : 'text-gray-400'"
                                                    x-text="selectedBarangName || '-- Pilih Barang --'"></span>
                                            ...
                                            <div x-show="barangOpen" @click.away="barangOpen = false"
                                                class="absolute z-[100] w-full mt-2 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden" x-cloak>
                                                <div class="p-2 border-b border-gray-50 bg-gray-50/50">
                                                    <input type="text" x-model="barangSearch" placeholder="Cari barang..."
                                                        class="w-full px-4 py-2 text-sm bg-white border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-blue-500">
                                                </div>
                                                <div class="max-h-48 overflow-y-auto custom-scrollbar">
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        <div class="bg-white/80 p-3 rounded-xl border border-blue-100">
                        ...
                        <div class="lg:col-span-7 space-y-8 relative z-10">
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    ...
                                    <div class="space-y-1.5">
                                        <label class="text-xs font-bold text-gray-500 uppercase ml-1">Harga Satuan</label>
                                        <div class="relative">
                                    ...
                                    <div class="md:col-span-2 pt-2 pb-1">
                                        <label class="inline-flex items-center cursor-pointer p-2 rounded-lg hover:bg-gray-50 transition-colors">
                                            <input type="checkbox" x-model="ada_rusak" class="form-checkbox h-5 w-5 text-red-500 rounded border-gray-300 focus:ring-red-500">
                                            <span class="ml-3 text-sm font-bold text-gray-700">Apakah ada barang yang rusak / bermasalah saat diterima?</span>
                                        </label>
                                    </div>
                                    ...
                                    <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5 p-5 bg-red-50/50 rounded-2xl border border-red-100 mb-2" 
                                        ...
                                        <div class="space-y-1.5">
                                            <label class="text-xs font-bold text-red-600 uppercase ml-1">Rusak Internal</label>
                                            ...
                                            <p class="text-[10px] text-red-500 font-medium ml-1">* Kesalahan sendiri (Dianggap rugi)</p>
                                        </div>
                                        
                                        <div class="space-y-1.5">
                                            <label class="text-xs font-bold text-orange-600 uppercase ml-1">Rusak Supplier (Return)</label>
                                            <input type="number" step="any" name="jumlah_return" x-model.number="return_qty"
                                            ...
                                            <p class="text-[10px] text-orange-500 font-medium ml-1">* Kesalahan supplier (Dapat di-return)</p>
                                        </div>
                                    </div>

                                    <div class="md:col-span-2 space-y-1.5">
                                        <label class="text-xs font-bold text-gray-500 uppercase ml-1">Stok Bersih (Sisa)</label>
                            ...
                            <button type="submit"
                                :disabled="isInvalidStok"
                                :class="isInvalidStok ? 'opacity-50 cursor-not-allowed grayscale' : 'hover:bg-yellow-700 hover:scale-[1.01] active:scale-95 shadow-lg shadow-yellow-200'"
                                class="w-full bg-yellow-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest transition-all flex items-center justify-center gap-3">
