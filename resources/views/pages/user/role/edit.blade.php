<x-layout.user.app title="Edit Role">
    <div class="py-2">
        <form action="{{ route('roles.update', $role->id) }}" method="POST"
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-gray-500 font-medium">Perbarui jabatan dan izin akses sistem Mirasa.</p>
                </div>
            </div>

            {{-- Input Nama Role --}}
            <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm border-l-4 border-amber-400">
                <div class="max-w-md">
                    <label for="name"
                    ...
                    <input type="text" id="name" name="name" required value="{{ old('name', $role->name) }}"
                        ...
                        class="w-full rounded-xl border-gray-200 bg-gray-50/50 py-3 px-4 text-gray-900 focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 focus:bg-white outline-none transition-all font-semibold uppercase">
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                <div class="relative w-full md:w-80">
                    <input type="text" id="permissionSearch" placeholder="Cari modul atau akses..."
                        class="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 outline-none transition-all text-sm font-medium">
                ...
                <div class="flex items-center gap-2 text-xs font-bold uppercase tracking-wider">
                    <button type="button" onclick="toggleAll(true)"
                        class="text-amber-600 hover:bg-amber-50 px-3 py-2 rounded-lg transition-all">Pilih
                        Semua</button>
                    <span class="text-gray-300">|</span>
                    <button type="button" onclick="toggleAll(false)"
                        class="text-gray-400 hover:bg-amber-50 px-3 py-2 rounded-lg transition-all">Reset</button>
                </div>
            </div>
                    ...
                    <div class="permission-group bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all overflow-hidden"
                        ...
                        <div class="px-6 py-4 bg-gray-50/50 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <button type="button" onclick="toggleGroup('{{ $group }}', true)"
                                class="text-[10px] font-extrabold text-amber-600 hover:text-amber-700 bg-amber-50 px-3 py-1.5 rounded-lg transition-colors tracking-tighter">
                                PILIH GRUP
                            </button>
                        </div>
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                                    @foreach ($items as $permission)
                                        @if (!preg_match('/(create|edit|delete|update|store|destroy|import|activate|print)/i', $permission->name))
                                            <label
                                                class="permission-item group flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-amber-200 hover:bg-amber-50/30 transition-all cursor-pointer"
                                                data-permission-name="{{ strtolower($permission->name) }}">
                                                <input type="checkbox" name="permissions[]"
                                                    value="{{ $permission->name }}" data-group="{{ $group }}"
                                                    {{ $role->hasPermissionTo($permission->name) ? 'checked' : '' }}
                                                    class="permission-checkbox view-checkbox w-4 h-4 rounded border-gray-300 text-amber-600 focus:ring-0 cursor-pointer transition-transform group-hover:scale-110">
                                                <span
                                                    class="text-xs font-bold text-gray-600 capitalize group-hover:text-amber-700 transition-colors">
            ...
            <div class="sticky bottom-6 z-50 mt-12">
                <div
                    ...
                    <div class="hidden md:block">
                        <p class="text-xs text-gray-400 font-medium">Periksa kembali perubahan sebelum memperbarui data
                            role.</p>
                    </div>
                    ...
                    <div class="flex gap-3 w-full md:w-auto">
                        ...
                        <button type="submit"
                            class="btn-submit flex-1 md:flex-none px-10 py-3 bg-amber-500 hover:bg-amber-600 text-white text-sm font-extrabold rounded-xl shadow-lg shadow-amber-200 transition-all active:scale-95 uppercase tracking-wider">
    ...
    <script>
        // Logika JavaScript tetap sama agar konsisten dengan create
