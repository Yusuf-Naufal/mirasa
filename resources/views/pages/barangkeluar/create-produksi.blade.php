    ...
    <div class="min-h-screen bg-gray-50/50 md:px-10 py-8"
        ...
        <div class="mx-auto flex flex-col pt-12">
            ...
            <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <a href="{{ route('barang-keluar.index') }}" class="group inline-flex items-center text-blue-600 hover:text-blue-700 text-sm font-semibold transition-all mb-2">
                        <svg class="h-4 w-4 mr-1 transform group-hover:-translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                        Kembali
                    </a>
                    <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Pengeluaran <span class="text-blue-600">FIFO</span></h1>
                    <p class="text-sm text-gray-500 font-medium italic">*Sistem akan mengambil batch tertua secara otomatis jika jumlah melebihi satu batch.</p>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="lg:col-span-5 space-y-6">
                        <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-4">
                            <label class="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Langkah 1: Pilih Barang</label>
                            <div class="relative">
                                ...
                                <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-2 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
                                    <input type="text" x-model="search" placeholder="Cari barang..." class="w-full px-4 py-3 border-b outline-none">
                                    <div class="max-h-60 overflow-y-auto">
                                        <template x-for="i in filtered" :key="i.id">
                                            <button type="button" @click="select(i)" class="w-full px-5 py-3 text-left hover:bg-blue-50 border-b border-gray-50 flex justify-between items-center transition-colors">
                                                <div>
                                                    <span class="font-bold text-gray-700 block" x-text="i.nama"></span>
                                                    <span class="text-[10px] text-gray-400" x-text="i.kode"></span>
                                                </div>
                                                <span class="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded-lg font-bold" x-text="'Stok: ' + i.stok_total"></span>
                                            </button>
                                        </template>
                                    </div>
                                </div>
                        ...
                        <div class="bg-blue-600 rounded-3xl p-6 text-white shadow-xl shadow-blue-200" x-show="selectedInventoryId">
                            ...
                            <div class="space-y-3">
                                <template x-if="simulasiFIFO.rencana.length === 0">
                                    <p class="text-xs text-blue-200 italic font-medium">Masukkan jumlah untuk melihat simulasi batch...</p>
                                </template>
                                
                                <template x-for="(r, index) in simulasiFIFO.rencana" :key="index">
                                    <div class="flex justify-between items-center bg-blue-500/50 p-3 rounded-xl border border-blue-400/30">
                                        <div class="flex flex-col">
                                            <span class="text-[10px] text-blue-200 font-bold uppercase">Batch Tgl: <span x-text="r.tgl" class="text-white"></span></span>
                            ...
                            <div class="mt-6 pt-4 border-t border-blue-400 flex justify-between items-center">
                                ...
                                <span class="text-xl font-black">Rp <span x-text="new Intl.NumberFormat('id-ID').format(simulasiFIFO.totalBiaya)"></span></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-7 space-y-6">
                        <div class="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                            <label class="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-6 block">Langkah 2: Detail Keluar</label>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <select name="id_proses" required class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500">
                                ...
                                <div class="md:col-span-2 space-y-2">
                                    <div class="flex justify-between items-center ml-1">
                                        <label class="text-xs font-bold text-blue-600 uppercase">Jumlah Pengeluaran</label>
                                        <span class="text-[10px] font-bold text-gray-400 uppercase">Tersedia: <span x-text="maxStok" class="text-blue-600"></span> <span x-text="selectedSatuan"></span></span>
                                    </div>
                                    <div class="relative">
                                        ...
                                        <span class="absolute right-6 top-1/2 -translate-y-1/2 font-bold text-blue-400 text-lg uppercase" x-text="selectedSatuan"></span>
                                    </div>
                            ...
                            <button type="submit" :disabled="jumlah > maxStok || jumlah <= 0 || !selectedInventoryId"
                                class="w-full mt-10 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed text-white py-5 rounded-3xl font-black uppercase tracking-widest shadow-xl shadow-blue-200 transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3">
